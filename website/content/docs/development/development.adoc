...
include::content/docs/development/container_setup.adoc[]

== How to develop in `ansible-oracle`?

IMPORTANT: This is an example how most of the development in `ansible-oracle` is done. +
A Pull-Request is only accepted, when all `github-Actions` are valid.


=== Prepare am Ansible Controller

IMPORTANT: `python3.11` or newer and `python-venv` are needed to complete the following setup. +
This could be done in a VM or WLS2 machine.

==== Software Packages

----
sudo apt install -y python3.11 python3.11-venv wget curl git
----
...
.Python-venv
----
python3.11 -m venv ~/venv/ansible-oracle
----
...
~/venv/ansible-oracle/python3 get-pip.py

~/venv/ansible-oracle/pip3 install -r ~/git/ansible-oracle/tools/dev/requirements_dev.txt
----

==== Execution-Environment

This step is only needed when `ansible-navigator` is used with an Execution-Environment. +
`ansible-builder` automatically detects if docker or podman is used.
Do not forget to complete the setup for docker or podman before starting `ansible-builder``.`

----
cd ~/git/ansible-oracle

~/venv/ansible-oracle/bin/ansible-builder build -v 3
----
...
.Skip `ansible-lint` during `pre-commit` execution
----
...
IMPORTANT: python3 should point to Python >=3.10, otherwise `ansible-navigator` and `ansible-lint` could not be used. +
